...
viz_map_attributable_number <- function(data_combine_exposure_response = data_combine_exposure_response,
  ...
  tmap_save(tm_an, file.path(outdir, sprintf("map_attributable_%04i_%04i.png", min(yy), max(yy))))
  
  return(c(
    file.path(outdir, sprintf("map_attributable_%04i_%04i.png", min(yy), max(yy)))
  ))
}