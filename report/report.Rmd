...
```{r plt-population, fig.cap = "Population by province."}
knitr::include_graphics(
  file.path("..", grep("population", targets::tar_read(fig_map_inputs_facet), value = T)), 
  error = F
)
```
...
knitr::include_graphics(
  file.path("..", grep("mortality", targets::tar_read(fig_map_inputs_facet), value = T)), 
  error = F
)
```
...
```{r plt-attributable-number, fig.cap = "Attributable deaths by province."}
knitr::include_graphics(
  file.path("..", targets::tar_read(fig_map_attributable_number)), error = F)
#   file.path("..", grep("attributable_[0-9]", targets::tar_read(fig_map_inputs_facet), value = T)), 
#   error = F
# )
```
...
knitr::kable(data_exposure_health[Year == 2020],
             caption = "Attributable number of deaths (PM2.5) by province for the year 2020 with average PM2.5 (μg/m3) population-weighted exposure for baseline case and counterfactual.",
