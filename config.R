...
out.geography <- file.path(outdir,
                           sprintf("IDN_provinces_%s.gpkg", runDate))
out.combined_data <- file.path(outdir,
...
datadir <- "~/CARDAT/"
indir.geography <- file.path(datadir, "CAR_staging_area",
                           "data_sharing/uploads_from_ivan/GADM_Global_Administrative_Areas/data_provided/")
infile.geography <- "gadm404-levels.gpkg"

indir.mort <- file.path(datadir, "CAR_staging_area", 
                        "IHME_GBD/IHME-GBD_2021_all_cause_mortality_SEA/data_provided/")
infile.mort <- "IHME-GBD_2021_DATA-SEA_all_cause_mortality.csv"

indir.pm25 <- file.path(datadir, "Environment_General",
                        "Air_pollution_model_GlobalGWR_PM25/GlobalGWR_PM25_V5GL02_1998_2020/data_provided/GWRPM25/Annual")
infile.pm25 <- sprintf("V5GL02.HybridPM25.Global.%04i01-%04i12.nc", yys_todo, yys_todo)
